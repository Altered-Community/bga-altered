...
class Notifications
{
    ...
    {
        self::notifyAll('removeActionCards', '', [
            'players_Id' => Players::getAll()->getIds(),
        ]);
    }
    ...
    public static function actionCardCleanup(
        $player,
        $type,
        $position,
        $cards,
        ...
        self::notifyAll(
            ...
            $msg ??
                clienttranslate(
                    '${player_name} places action card ${card_type} at position ${position} (finishing action)'
                ),
            [
                'i18n' => ['card_type'],
                'player' => $player,
    ...
    public static function buyBuilding($player, $cost, $building, $card)
    {
        ...
        $msg = clienttranslate(
            '${player_name} pays ${amount_money} for building ${building_name}'
        );
        if (is_null($cost)) {
            $msg = clienttranslate(
                '${player_name} adds ${building_name} for free'
            );

            if (!is_null($card)) {
                $msg = clienttranslate(
                    '${player_name} adds ${building_name} (${card_name})'
                );
                $data['card'] = $card;
            }
        }
    ...
    public static function upgradeCard($player, $card)
    {
        self::notifyAll(
            'upgradeCard',
            clienttranslate(
                '${player_name} upgrades ${action_card_name}${action_card_icon}${action_card_level}'
            ),
            [
                'player' => $player,
                'actionCard' => $card,
    ...
    public static function placeWorkersProject(
        $player,
        $strength,
        $workers,
        $project,
        ...
        self::notifyAll(
            'slideMeeples',
            clienttranslate(
                '${player_name} supports a conservation project on ${slot} : ${card_name}'
            ),
            [
                'player' => $player,
                'strength' => $strength,
    ...
    public static function donation($player, $cost, $meeple)
    {
        self::notifyAll(
            'donation',
            \clienttranslate(
                '${player_name} donates ${bonuses_desc} to get ${bonuses2_desc}'
            ),
            [
                'player' => $player,
                'bonuses' => [MONEY => -$cost],
    ...
    public function slideMeeples($meeples)
    {
    ...
    public static function updateIncome($player)
    {
        self::notifyAll('updateIncome', '', [
            'player' => $player,
            'income' => $player->getMoneyIncome(),
        ]);
    }

    public static function removeScoreboardBonus($tileBonuses)
    {
        self::notifyAll('removeScoreboardBonus', '', [
            'conservationBonuses' => $tileBonuses,
        ]);
    }
    ...
    //                      |_|
    /////////////////////////////////
    public static function setupPlayer(
        $player,
        $mapId,
        $cards,
        $meeples,
        ...
        self::notifyAll(
            'setupPlayer',
            \clienttranslate('${player_name} will play Map ${mapId}'),
            [
                'player' => $player,
                'mapId' => $mapId,
    ...
    public function advanceBreak($player, $n, $break, $maxBreak)
    {
        $data = [
            'player' => $player,
            'n' => $n,
        ...
        if ($break == $maxBreak) {
            ...
            $msg = clienttranslate(
                '${player_name} advances break token of ${n} space(s), now at ${break}/${maxBreak}'
            );
        }
    ...
    public function startBreak()
    {
        self::notifyAll(
            'startBreak',
            \clienttranslate('Starting a new break'),
            []
        );
    }
    ...
    public static function breakCleanupTokens($tokens)
    {
        self::notifyAll(
            'discardTokens',
            clienttranslate('All tokens are removed from player cards'),
            [
                'meeples' => $tokens,
            ]
        );
    ...
    public function breakReturnWorkers($workers)
    {
        self::notifyAll(
            'slideMeeples',
            clienttranslate('All workers go back to each player\'s reserve'),
            [
                'meeples' => $workers->toArray(),
            ]
        );
    ...
    public function breakRefill($meeples)
    {
        self::notifyAll(
            'addMeeples',
            clienttranslate('Replenishing partner zoos and universities'),
            [
                'meeples' => $meeples->toArray(),
            ]
        );
    ...
    public function finishBreak()
    {
        self::notifyAll(
            'finishBreak',
            \clienttranslate('End of the break'),
            []
        );
    }

    public static function finalScoring(
        $player,
        $score,
        $newScore,
        $appeal,
    ...
    public static function endOfGame()
    {
        self::notifyAll(
            'endOfGame',
            clienttranslate('End of game triggered'),
            []
        );
    }
    ...
    public static function drawCards(
        $player,
        $cards,
        $privateMsg = null,
        $publicMsg = null,
        ...
        self::notifyAll(
            ...
            $publicMsg ??
                clienttranslate(
                    '${player_name} draws ${n} card(s) from the deck'
                ),
            $args + [
                'player' => $player,
                'n' => count($cards),
    ...
    public static function initialDraw($player, $cards, $scoringCards)
    {
        ...
        self::drawCards(
            ...
            $scoringCards,
            clienttranslate(
                'You draw ${card_names} from the deck (scoring cards)'
            ),
            clienttranslate(
                '${player_name} draws ${n} scoring cards from the deck'
            ),
            [
                'scoringCard' => true,
            ]
    ...
    public static function snapCard($player, $card)
    {
        self::notifyAll(
            'snapCard',
            clienttranslate(
                '${player_name} snaps ${card_names} from the display'
            ),
            [
                'player' => $player,
                'cards' => [$card],
    ...
    public static function takeCardInRange($player, $card)
    {
        self::notifyAll(
            'snapCard',
            clienttranslate(
                '${player_name} takes ${card_names} in reputation range from the display'
            ),
            [
                'player' => $player,
                'cards' => [$card],
    ...
    public static function sponsorMagnet($player, $cards)
    {
        self::notifyAll(
            'sponsorMagnet',
            clienttranslate(
                '${player_name} takes ${card_names} from the display (Sponsor magnet effect)'
            ),
            [
                'player' => $player,
                'cards' => $cards->toArray(),
    ...
    public static function fillPool($cards, $pool)
    {
        self::notifyAll(
            'fillPool',
            clienttranslate('The display is replenished with ${card_names}'),
            [
                'cards' => $cards,
                'pool' => $pool->toArray(),
            ]
    ...
    public static function discardCards(
        $player,
        $cards,
        $privateMsg = null,
        $publicMsg = null,
        ...
        self::notifyAll(
            'discardCards',
            $publicMsg ??
                clienttranslate('${player_name} discards ${n} card(s)'),
            $args + [
                'player' => $player,
                'n' => count($cards),
            ]
    ...
    public static function discardCardsOnDisplay($player, $cards, $msg = null)
    {
        self::notifyAll(
            ...
            $msg ??
                clienttranslate(
                    '${player_name} discards ${card_names} from display'
                ),
            [
                'player' => $player,
                'cards' => $cards->toArray(),
    ...
    public static function discardPoolCardsBreak($cards)
    {
        self::notifyAll(
            'discardCardsOnDisplay',
            \clienttranslate(
                'Removing first two cards of the display: ${card_names}'
            ),
            [
                'cards' => $cards->toArray(),
            ]
    ...
    public static function discardProject($card, $tokenIds)
    {
        self::notifyAll(
            'discardCardsOnDisplay',
            clienttranslate(
                'The rightmost project card is discarded: ${card_names}'
            ),
            [
                'cards' => [$card],
                'tokenIds' => $tokenIds,
    ...
    public static function moveProjects($player, $card, $cards, $fromDisplay)
    {
        self::notifyAll(
            ...
            $fromDisplay
                ? clienttranslate(
                    '${player_name} buys a new conservation project from display: ${card_names}'
                )
                : clienttranslate(
                    '${player_name} plays a new conservation project: ${card_names}'
                ),
            [
                'player' => $player,
                'cards' => [$card],
    ...
    public static function buyAnimal(
        $player,
        $animal,
        $cost,
        $enclosure,
        ...
        $msg = is_null($enclosure)
            ? clienttranslate(
                '${player_name} plays ${card_name} for ${amount_money} and places it using Flocking ability'
            )
            : clienttranslate(
                '${player_name} plays ${card_name} for ${amount_money} and places it in ${building_name}'
            );
    ...
    public static function moveAnimal(
        $player,
        $animal,
        $enclosure,
        $freeEnclosure
        ...
        self::notifyAll(
            'moveAnimal',
            clienttranslate(
                '${player_name} moves ${card_name} into the ${building_name} and free ${building_name2}'
            ),
            [
                'player' => $player,
                'card' => $animal,
    ...
    public static function playSponsor(
        $player,
        $sponsor,
        $meeples,
        $fromDisplay
        ...
        self::notifyAll(
            ...
            $fromDisplay
                ? clienttranslate(
                    '${player_name} buys ${card_name} from display'
                )
        ...
        if (is_null($msg)) {
            ...
            if (!is_null($source)) {
                if ($source instanceof \ARK\Models\ZooCard) {
                    $msg = clienttranslate(
                        '${player_name} gains ${bonuses_desc} (${card_name})'
                    );
                    ...
                    $msg = clienttranslate(
                        '${player_name} gains ${bonuses_desc} (${source})'
                    );
                    $args['source'] = $source;
                    $args['i18n'][] = 'source';
                }
    ...
    public static function incReputation(
        $player,
        $amount,
        $total,
        $source = null
    ...
    public static function incConservation(
        $player,
        $amount,
        $total,
        $source = null
    ...
    public static function payMoney($player, $amount, $total, $source)
    {
        self::notifyAll(
            'getBonuses',
            clienttranslate(
                '${player_name} pays ${bonuses_desc} for ${source}'
            ),
            [
                'i18n' => ['source'],
                'player' => $player,
    ...
    public static function sprint($player, $cards)
    {
        self::drawCards(
            ...
            clienttranslate(
                '${player_name} draws ${n} card(s) for sprint effect'
            )
        );
    }
    ...
    public static function preHunter($player, $cards)
    {
        self::drawCards(
            ...
            clienttranslate(
                '${player_name} draws ${card_names} for hunter effect'
            ),
            ['cards' => $cards->toArray()]
        );
    }
    ...
    public static function hunter($player, $cardsToDiscard, $card)
    {
        ...
        self::discardCards(
            ...
            $noDiscard
                ...
                : clienttranslate(
                    'You keep ${card_name} and discard ${card_names} for hunter effect'
                ),
            $noDiscard
                ? clienttranslate(
                    '${player_name} keep ${card_name} card for hunter effect'
                )
                : clienttranslate(
                    '${player_name} keep ${card_name} card and discard ${n} card(s) for hunter effect'
                ),
            ['card' => $card],
            ['card' => $card]
        );
    ...
    public static function failHunter($player, $cardsToDiscard)
    {
        self::discardCards(
            ...
            clienttranslate(
                '${player_name} discard all ${n} card(s) of hunter effect (no animals)'
            )
        );
    }
    ...
    public static function prePerception($player, $cards, $draw, $keep)
    {
        self::drawCards(
            ...
            clienttranslate(
                '${player_name} draws ${n} card(s) for perception effect'
            )
        );
    }
    ...
    public static function perception($player, $cardsToDiscard, $cardsToKeep)
    {
        self::notifyAll(
            'discardCards',
            clienttranslate(
                '${player_name} keep ${m} card(s) and discard ${n} card(s) for perception effect'
            ),
            [
                'player' => $player,
        ...
        self::notify(
            ...
            'pDiscardCards',
            clienttranslate(
                'You keep ${card_names2} and discard ${card_names}'
            ),
            [
                'player' => $player,
    ...
    public static function sunbathing($player, $cardsToSell, $money)
    {
        self::discardCards(
            ...
            clienttranslate(
                '${player_name} sells ${n} card(s) for ${bonuses_desc}'
            ),
            [
                'bonuses' => [MONEY => $money],
            ]
    ...
    public static function pouch($player, $cardsToPouch, $appeal)
    {
        self::discardCards(
            ...
            $cardsToPouch,
            clienttranslate(
                'You pouch ${card_names} cards for ${bonuses_desc}'
            ),
            clienttranslate(
                '${player_name} pouch ${n} card(s) for ${bonuses_desc}'
            ),
            [
    ...
    public static function digging($player, $type, $cardToDiscard, $card)
    {
        if ($type == 'hand') {
            ...
            self::discardCardsOnDisplay(
                ...
                $cardToDiscard,
                clienttranslate(
                    '${player_name} digs ${card_names} from the display'
                )
            );
        }
    ...
    public static function preScavenging($player, $cards)
    {
        self::drawCards(
            ...
            $cards,
            clienttranslate(
                'You draw ${card_names} from discard for scavenging effect'
            ),
            clienttranslate(
                '${player_name} draws ${n} card(s) from discard for scavenging effect'
            )
        );
    }
    ...
    public static function scavenging($player, $cardsToDiscard, $card)
    {
        self::discardCards(
            ...
            $cardsToDiscard,
            clienttranslate(
                'You keep ${card_name} and discard ${card_names} for scavenging effect'
            ),
            clienttranslate(
                '${player_name} keep 1 card and discard ${n} card(s) for scavenging effect'
            ),
            [],
            ['card' => $card]
        );
    ...
    public static function clever($player, $type, $position, $cards)
    {
        self::actionCardCleanup(
            ...
            $cards,
            clienttranslate(
                '${player_name} places ${card_type} at position ${position} (Clever effect)'
            )
        );
    }
    ...
    public static function preResistance($player, $cards)
    {
        self::drawCards(
            ...
            clienttranslate(
                '${player_name} draws ${n} scoring cards for resistance effect'
            ),
            ['scoringCard' => true]
        );
    }
    ...
    public static function resistance($player, $cardsToDiscard, $card)
    {
        self::discardCards(
            ...
            $cardsToDiscard,
            clienttranslate(
                'You keep ${card_name} and discard ${card_names} for resistance effect'
            ),
            clienttranslate(
                '${player_name} keep 1 scoring card and discard ${n} scoring card for resistance effect'
            ),
            ['scoringCard' => true],
    ...
    public static function dominance($player, $card)
    {
        ...
        self::drawCards(
            ...
            clienttranslate(
                '${player_name} add ${card_names} conservation project to its hand with Dominance'
            ),
            ['cards' => $cards]
        );
    }
    ...
    public static function boost($player, $type, $position, $cards)
    {
        self::actionCardCleanup(
            ...
            $cards,
            clienttranslate(
                '${player_name} places ${card_type} at position ${position} (Boost effect)'
            )
        );
    }
    ...
    public static function constriction($player, $meeples, $players)
    {
        self::notifyAll(
            'addMeeples',
            clienttranslate(
                '${player_name} use Constriction effect and gives constriction token(s) to ${players_names}'
            ),
    ...
    public static function venom($player, $meeples, $players)
    {
        self::notifyAll(
            'addMeeples',
            clienttranslate(
                '${player_name} use Venom effect and gives Venom token(s) to ${players_names}'
            ),
            [
                'player' => $player,
                'meeples' => $meeples,
    ...
    public static function useMultiplier($player, $meeple)
    {
        self::notifyAll(
            'discardTokens',
            clienttranslate('${player_name} uses a multiplier token'),
            [
                'player' => $player,
                'meeples' => [$meeple],
            ]
    ...
    public static function discardToken(
        $player,
        $type,
        $meeple,
        $silent = false
        ...
        self::notifyAll(
            ...
            $silent
                ? ''
                : clienttranslate('${player_name} discard its ${type} token'),
            [
                'player' => $player,
                'meeples' => [$meeple],
                'type' => $type,
    ...
    public static function useReductionTokens($player, $tokensUsed)
    {
        self::notifyAll(
            'discardTokens',
            clienttranslate(
                '${player_name} uses ${n} token(s) from sponsor card(s)'
            ),
            [
                'player' => $player,
                'n' => count($tokensUsed),
    ...
    public static function hypnosis($player, $target)
    {
        self::notifyAll(
            'hypnosis',
            clienttranslate(
                '${player_name} chooses to hypnotize ${player_name2}'
            ),
            [
                'player' => $player,
                'player2' => $target,
    ...
    {
        $msg = clienttranslate(
            '${player_name} chooses ${player_name2} to Pilfer'
        );
        if (!is_null($target1) && !is_null($target2)) {
            $msg = clienttranslate(
                '${player_name} chooses ${player_name2} and ${player_name3} to Pilfer'
            );
        }
    ...
    public static function pilferingCard($player, $card, $otherPlayer)
    {
        ...
        self::notifyAll(
            'pilferingCard',
            clienttranslate(
                '${player_name} gives 1 card to ${player_name2} from Pilfering effect'
            ),
            [
                'player' => $player,
                'player2' => $otherPlayer,
        ...
        self::notify(
            $player,
            'pDiscardCards',
            'You give ${card_names} from the Pilfering effect',
            [
                'player' => $player,
        ...
        self::notify(
            $otherPlayer,
            'pDrawCards',
            clienttranslate('You get ${card_names} from the Pilfering effect'),
            [
                'player' => $otherPlayer,
    ...
    public static function pilferingMoney($player, $amount, $otherPlayer)
    {
        self::notifyAll(
            'pilferingMoney',
            clienttranslate(
                '${player_name} gives ${bonuses_desc} to ${player_name2} for Pilfering effect'
            ),
            [
                'player' => $player,
                'player2' => $otherPlayer,
    ...
    public static function gainWorker($player, $worker)
    {
        self::notifyAll(
            'slideMeeples',
            clienttranslate('${player_name} gains a new Association worker'),
            [
                'player' => $player,
                'meeples' => [$worker],
            ]
    ...
    public static function map8Effect($player, $cards)
    {
        self::drawCards(
            ...
            $cards,
            clienttranslate(
                'You draw ${card_names} from the deck (Map 8 effect)'
            ),
            clienttranslate(
                '${player_name} draws ${card_names} sponsor card(s) from the deck (Map 8 effect)'
            ),
            ['cards' => $cards->toArray()]
        );
    }
    ...
    public static function wazaSpecial($player, $type, $card)
    {
        ...
        self::drawCards(
            ...
            $cards,
            clienttranslate(
                'You draw ${card_names} from the deck (Waza Special Assignement effect)'
            ),
            clienttranslate(
                '${player_name} draws ${card_names} animal card from the deck (Waza Special Assignement effect)'
            ),
            ['cards' => $cards]
        );
    }
        ...
        if (isset($data['cards'])) {
            ...
            foreach ($data['cards'] as $i => $card) {
                ...
                $args['card_name_' . $i] = [
                    ...
                    'args' => [
                        'i18n' => ['card_name'],
                        'card_name' => is_array($card)
                            ? $card['name']
                            : $card->getName(),
                        'card_id' => is_array($card)
                            ? $card['id']
                            : $card->getId(),
                        'preserve' => ['card_id'],
                    ],
