<?php
const STARTER_NEVENKA = [
  'LY_Common_NevenkaBlotch' => 1,
  'LY_Rare_Martengale' => 3,
  'LY_Common_MountainGuide' => 2,
  'LY_Rare_KadigiranMageDancer' => 3,
  'LY_Common_SnowballCommando' => 3,
  'LY_Rare_OuroborosInkcaster' => 3,
  'LY_Common_RekaGuide' => 2,
  'LY_Common_Hathor' => 2,
  'LY_Common_Tanuki' => 2,
  'LY_Rare_Dapeng' => 2,
  'LY_Rare_MagicalTraining' => 2,
  'LY_Common_TwinkleTwinkle' => 2,
  'LY_Common_LyraSignet' => 2,
  'LY_Common_XMarkstheSpot' => 2,
  'LY_Common_ClothCocoon' => 2,
  'LY_Rare_ParliamentofAshes' => 2,
  'LY_Common_OuroborosTrickster' => 3,
  'LY_Common_Esmeralda' => 2,
];
?>
